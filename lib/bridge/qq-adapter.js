...
export function createQQAdapter({ appID, appSecret, agentId, onMessage, dmGuildMap, onDmGuildDiscovered, onStatus }) {
  ...
  function handleEvent(type, data) {
    ...
    if (type === "C2C_MESSAGE_CREATE") {
      ...
      // 提取被引用消息的内容（用户引用文件时使用）
      let quotedAttachments = [];
      let quotedText = "";
      if (data.message_reference) {
        const ref = data.message_reference;
        if (ref.content) {
          quotedText = ref.content.trim();
        }
        if (ref.attachments) {
          quotedAttachments = extractAttachments({ attachments: ref.attachments });
        }
      }
      
      // 合并附件
      const allAttachments = quotedAttachments.length ? [...quotedAttachments, ...attachments] : attachments;
      // 合并文本
      const fullText = quotedText && text ? `${quotedText}\n${text}` : (quotedText || text);
      
      if (!fullText && !allAttachments.length) return;
      if (fullText.length > MAX_MSG_SIZE) return;
      ...
      onMessage({
        ...
        text: fullText.slice(0, MAX_MSG_SIZE),
